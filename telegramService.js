...
function inicializarBot() {
    ...
    try {
        // Em produção (Render), usa webhook. Em desenvolvimento, usa polling
        if (isProduction && WEBHOOK_URL) {
            // Webhook para produção (Render)
            bot = new TelegramBot(TELEGRAM_TOKEN);
            const webhookPath = `/telegram-webhook/${TELEGRAM_TOKEN}`;
            const fullWebhookUrl = `https://${WEBHOOK_URL}${webhookPath}`;

            bot.setWebHook(fullWebhookUrl)
                .then(() => {
                    console.log('✅ Bot do Telegram inicializado com WEBHOOK!');
            ...
            console.log('✅ Bot do Telegram inicializado com POLLING (dev)!');
        }
...
/**
 * Retorna a instância do bot (necessário para webhook)
 */
function getBot() {
    return bot;
}
...
/**
 * Retorna o token (necessário para validar webhook)
 */
function getToken() {
